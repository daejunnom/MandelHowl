...
pub fn validate_modes_v1(bytes: &[u8]) -> Result<ModesSummary, String> {
    ...
    let mut first_mode_id = String::new();
    let mut last_mode_id = String::new();
    for index in 0..count {
        ...
        for (offset, label) in [
            (56, "actuator coupling"),
            (64, "microphone coupling"),
            (72, "radiation efficiency"),
            (80, "phase reference"),
        ] {
            f64_le(bytes, base + offset, label)?;
        }
        ...
        if index == 0 {
            first_mode_id = mode_id.clone();
        }
        last_mode_id = mode_id;
        previous_frequency = natural_frequency;
    }

    let minimum_frequency_hz = f64_le(bytes, HEADER_BYTES + 32, "minimum frequency")?;
    Ok(ModesSummary {
        count,
        first_mode_id,
        last_mode_id,
        minimum_frequency_hz,
        maximum_frequency_hz: previous_frequency,
    })
}
...
pub fn validate_response_v1(bytes: &[u8]) -> Result<ResponseSummary, String> {
    ...
    let mut previous_frequency = 0.0;
    let mut minimum_frequency = 0.0;
    for index in 0..sample_count {
        let base = HEADER_BYTES + index * RESPONSE_RECORD_BYTES;
        let frequency = f64_le(bytes, base, "response frequency")?;
        f64_le(bytes, base + 8, "response real component")?;
        f64_le(bytes, base + 16, "response imaginary component")?;
